...
packer.use({
	...
	config = function()
		...
		vim.keymap.set('n', '<leader>R', "<cmd>lua require'dap'.run_to_cursor()<cr>", { desc = 'nvim-dap | Run to cursor' })
		vim.keymap.set(
			'n',
			'<leader>B',
		...
		vim.keymap.set('n', '<leader>S', "<cmd>lua require'dap.ui.widgets'.scopes()<cr>", { desc = 'nvim-dap | Scopes' })
