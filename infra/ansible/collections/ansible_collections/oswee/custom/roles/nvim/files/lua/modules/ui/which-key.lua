...
local keymaps = {
	...
	z = {
		name = '+notes',
		c = { '<Cmd>ZkCd<CR>', 'CD to notebook root' },
		i = { '<Cmd>ZkIndex<CR>', 'Index the notebook' },
		b = { '<Cmd>ZkBacklinks<CR>', 'Open backlinks' },
		n = { "<Cmd>ZkNew { title = vim.fn.input('Title: ') }<CR>", 'New note' },
		o = { "<Cmd>ZkNotes { sort = { 'modified' } }<CR>", 'Open note' },
		f = { "<Cmd>ZkNotes { sort = { 'modified' }, match = vim.fn.input('Search: ') }<CR>", 'By search term' },
		t = { '<Cmd>ZkTags<CR>', 'Filter notes by tag' },
		m = { "<Cmd>'<,'>ZkMatch<CR>", 'Last visual selection' },
	},
	f = {
		name = '+telescope',
	...
	s = {
		name = '+symbols',
		t = { '<cmd>SymbolsOutline<cr>', 'Toggle symbols' },
